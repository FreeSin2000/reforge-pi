...
1. **知识外化到文件** —— 会话会丢，文件不会；
2. **探索优先，沉淀延后** —— 边翻代码边提问、尝试解释，过程频繁记进 `journal.md`；固定文档（01~04、`_concepts/`）只在积累到检查点后统一形成；
3. **知识与方法分离** —— 项目细节放 `notes/<project>/`，可迁移的抽象放 `notes/_concepts/`；
4. **先静态后动态，先主干后边角** —— 别一上来就上 gdb，别纠缠错误处理。
