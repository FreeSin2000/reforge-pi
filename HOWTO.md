...
**产物阶梯**：journal 条目（探索过程，频繁）→ 综合检查点（聚类/消解矛盾）→ 结构化笔记（01~04）→ 摘要/调用图 → 能白板复述的模型。
每上一级，都要落盘。
...
- **随手问会自动记进 journal**：探索期的问题 / 假设 / 发现都进 `notes/<project>/journal.md`（见 5.12）；固定文档等积累到检查点才沉淀。
...
四份是**沉淀目标**，不要一次写完：探索期结论先进 `journal.md`，到综合检查点再沉淀。建议 01/02 起手做粗版，03/04 积累后写。
...
每次综合检查点后，回头把 `index.md` 的 TL;DR 提炼到 3~5 行。
...
- 定位并用 `cscope` / `rg` 给出 `src/linux-6.12/file:line`；
- 每条「问题 / 假设 / 发现 / 困惑 / 反例」在 `notes/linux-6.12/journal.md` 追加一条 `J-NNNN`，含**尝试解释 + 置信度 + 状态**；
- **默认不动 01~04**；某个主题攒够（≥5 条）或你说「总结」时，进入综合检查点统一沉淀。
...
- 文件名固定：`index.md`、`journal.md` + `01-structure` … `04-debug-plan`。
...
> `src/` **不入库**。克隆本仓库到新机器后，需自行把源码下载/克隆到 `src/<project>/`。
