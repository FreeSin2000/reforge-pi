...
- **随手问会自动记**：当回答构成可复用结论时，pi 会追加到 `notes/<project>/journal.md`（见 5.12）；你说「记一下」强制记、「只看看」则不记。
...
- 对「形成理解的结论」在 `notes/linux-6.12/journal.md` 追加一条 `J-NNNN`；
- 稳定结论 promote 进 01~04 / `glossary` / `map` / `_concepts/`，并在 journal 留 `→` 指针；
- 纯查位置 / 一次性问题**不记**（除非进 glossary）。
